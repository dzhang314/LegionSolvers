...
Legion::Rect<DIM, COORD_T> create_rect(std::size_t size) {
    if constexpr (DIM == 1) { return {0, static_cast<COORD_T>(size) - 1}; }
    if constexpr (DIM == 2) {
        return {
            {0, 0},
...
void test_task(
    ...
    Legion::IndexSpaceT<DIM, COORD_T> index_space =
        rt->create_index_space(ctx, index_rect);

    Legion::Rect<COLOR_DIM, COLOR_COORD_T> color_rect =
        create_rect<COLOR_DIM, COLOR_COORD_T>(10);
    Legion::IndexSpaceT<COLOR_DIM, COLOR_COORD_T> color_space =
        rt->create_index_space(ctx, color_rect);

    Legion::IndexPartitionT<DIM, COORD_T> index_partition =
        rt->create_equal_partition(ctx, index_space, color_space);

    {
        LegionSolvers::PartitionedVector<ENTRY_T> u(
            ctx, rt, "u", index_partition
        );
        LegionSolvers::PartitionedVector<ENTRY_T> v(
            ctx, rt, "v", index_partition
        );
        LegionSolvers::PartitionedVector<ENTRY_T> w(
            ctx, rt, "w", index_partition
        );

        u.constant_fill(1.5); // TODO: random_fill
        v.constant_fill(2.7); // TODO: random_fill
        w = u;
