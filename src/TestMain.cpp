...
void top_level_task(const Legion::Task *,
    ...
    for (Legion::coord_t vector_size = 1; vector_size <= 1000; ++vector_size) {
        for (Legion::coord_t num_colors = 1; num_colors <= 20; ++num_colors) {

            const Legion::IndexSpaceT<1> index_space =
                rt->create_index_space(ctx, Legion::Rect<1>{0, vector_size - 1});
            const Legion::IndexSpaceT<1> color_space =
                rt->create_index_space(ctx, Legion::Rect<1>{0, num_colors - 1});

            LegionSolvers::DistributedVectorT<double> u{"u", index_space, color_space, ctx, rt};
            LegionSolvers::DistributedVectorT<double> v{"v", u.index_partition, ctx, rt};
            LegionSolvers::DistributedVectorT<double> w{"w", u.index_partition, ctx, rt};
            LegionSolvers::DistributedVectorT<double> x{"x", u.index_partition, ctx, rt};
