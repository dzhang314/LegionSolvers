...
namespace LegionSolvers {
    ...
    class SquarePlanner {
        ...
        std::vector<DenseDistributedVector<ENTRY_T>> sol_vectors;
        std::vector<DenseDistributedVector<ENTRY_T>> rhs_vectors;
        std::vector<
            std::vector<DenseDistributedVector<ENTRY_T>> // vector over spaces
        > workspace_vectors; // vector over num workspace
    ...
    public:
        ...
        std::size_t add_sol_vector(const DenseDistributedVector<ENTRY_T> &v) {
            ...
            if (canonical_index_partitions.size() > idx) {
                assert(canonical_index_partitions[idx] ==
                       v.get_index_partition());
            } else {
        ...
        std::size_t add_rhs_vector(const DenseDistributedVector<ENTRY_T> &v) {
            assert(workspace_fields == Legion::FieldSpace::NO_SPACE);
            assert(workspace_regions.empty());
            assert(workspace_partitions.empty());
            assert(workspace_vectors.empty());
        ...
        void allocate_workspace(std::size_t num_vectors) {
            ...
            workspace_fields = LegionSolvers::create_field_space(ctx, rt,
                field_sizes, field_ids
            );
            for (std::size_t i = 0; i < num_vectors; ++i) {
                workspace_vectors.emplace_back();
            }
            for (std::size_t i = 0; i < num_spaces; ++i) {
                workspace_regions.push_back(rt->create_logical_region(ctx,
                    canonical_index_spaces[i], workspace_fields
                ));
                workspace_partitions.push_back(rt->get_logical_partition(ctx,
                    workspace_regions.back(), canonical_index_partitions[i]
                ));
                for (std::size_t j = 0; j < num_vectors; ++j) {
                    workspace_vectors[j].emplace_back(ctx, rt,
                        "workspace_" + std::to_string(j) +
                                 "_" + std::to_string(i),
        ...
        void add_row_partitioned_matrix(
            ...
            std::size_t domain_index, std::size_t range_index
        ) {
            ...
            const Legion::IndexPartition ghost_partition =
                matrix.domain_partition_from_kernel_partition(
                    canonical_index_spaces[domain_index],
                    kernel_partition
                );
            row_partitioned_matrices.emplace_back(
                &matrix, domain_index, range_index, kernel_partition,
        ...
        DenseDistributedVector<ENTRY_T> &get_vector(
            std::size_t vec_idx,
        ...
        void zero_fill(std::size_t vec_idx) {
            if (vec_idx == 0) {
                for (DenseDistributedVector<ENTRY_T> &v : sol_vectors) {
                    v.zero_fill();
                }
            } else if (vec_idx == 1) {
                for (DenseDistributedVector<ENTRY_T> &v : rhs_vectors) {
                    v.zero_fill();
                }
            } else {
                for (DenseDistributedVector<ENTRY_T> &v :
        ...
        void axpy(
            ...
            for (std::size_t i = 0; i < num_spaces; ++i) {
                get_vector(dst_index, i).axpy(
                    numer, denom, get_vector(src_index, i)
                );
            }
        }
        ...
        Scalar<ENTRY_T> dot(
            ...
            const std::size_t num_spaces = get_num_spaces();
            Scalar<ENTRY_T> result =
                get_vector(v_idx, 0).dot(get_vector(w_idx, 0));
            for (std::size_t i = 1; i < num_spaces; ++i) {
                result = result +
                    get_vector(v_idx, i).dot(get_vector(w_idx, i));
            }
            return result;
        }
