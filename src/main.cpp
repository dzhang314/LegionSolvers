...
void top_level_task(const Legion::Task *,
    ...
    { // mov x, 0
        Legion::IndexLauncher init_launcher{ZERO_FILL_TASK_ID, color_is,
    ...
    // dot r_norm2, r, r
    Legion::TaskLauncher dot_product_launcher_1{
        DOT_PRODUCT_TASK_ID, Legion::TaskArgument{nullptr, 0}};
    dot_product_launcher_1.add_region_requirement(Legion::RegionRequirement{
    ...
    for (int i = 0; i < 16; ++i) {
        ...
        Legion::TaskLauncher division_launcher_1{
            DIVISION_TASK_ID, Legion::TaskArgument{nullptr, 0}};
        ...
        { // axpy x, +alpha, p
            Legion::IndexLauncher axpy_launcher{
                AXPY_TASK_ID, color_is, Legion::TaskArgument{nullptr, 0},
                arg_map};
        ...
        // axpy r, -alpha, q
        Legion::TaskLauncher negation_launcher{
            NEGATION_TASK_ID, Legion::TaskArgument{nullptr, 0}};
        ...
        Legion::Future r_norm2_new =
            rt->execute_task(ctx, norm_squared_launcher);

        // div beta, r_norm2_new, r_norm2
        Legion::TaskLauncher division_launcher_2{
            DIVISION_TASK_ID, Legion::TaskArgument{nullptr, 0}};
        ...
        { // aypx p, +beta, r
            Legion::IndexLauncher aypx_launcher{
                AYPX_TASK_ID, color_is, Legion::TaskArgument{nullptr, 0},
                arg_map};
...
void check_task(const Legion::Task *task,
    ...
    for (Legion::PointInRectIterator<1> iter(rect); iter(); iter++) {
        ...
        const double r =
            (iter[0] == max_elements - 1) ? 0.0 : src_acc[*iter + 1];
...
template <typename TReturn,
          TReturn (*TaskPtr)(const Legion::Task *,
                             const std::vector<Legion::PhysicalRegion> &,
                             Legion::Context, Legion::Runtime *)>
void register_cpu_task(Legion::TaskID task_id, const char *task_name) {
    Legion::TaskVariantRegistrar registrar(task_id, task_name);
    registrar.add_constraint(
        Legion::ProcessorConstraint{Legion::Processor::LOC_PROC});
    Legion::Runtime::preregister_task_variant<TReturn, TaskPtr>(registrar,
                                                                task_name);
}
...
int main(int argc, char **argv) {
    ...
    register_cpu_task<double, dot_product_task<double>>(DOT_PRODUCT_TASK_ID,
                                                        "dot_product");
    register_cpu_task<axpy_task<double>>(AXPY_TASK_ID, "axpy");
