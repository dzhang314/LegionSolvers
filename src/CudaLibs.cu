...
cudaStream_t CUDALibraries::get_stream() {
  if (this->stream_ == nullptr) {
    CHECK_CUDA(cudaStreamCreateWithFlags(&this->stream_, cudaStreamNonBlocking));
  }
  return this->stream_;
}
...
void LoadCUDALibsTask::preregister(bool verbose) {
  if (verbose) {
    std::cout << "[LegionSolvers] Registering task "
              << std::string(LoadCUDALibsTask::task_name)
              << " with ID " << LoadCUDALibsTask::TASK_ID << "." << std::endl;
  }
  Legion::TaskVariantRegistrar registrar{LoadCUDALibsTask::TASK_ID, LoadCUDALibsTask::task_name};
...
void loadCUDALibs(Legion::Context ctx, Legion::Runtime* runtime) {
  auto tunable = Legion::Mapping::DefaultMapper::DefaultTunables::DEFAULT_TUNABLE_GLOBAL_GPUS;
  auto num_gpus = runtime->select_tunable_value(ctx, tunable, LEGION_SOLVERS_MAPPER_ID).get<size_t>();
  if (num_gpus == 0) return;
  // Launch the initialization task onto each GPU.
  auto launch_space = runtime->create_index_space(ctx, Legion::Rect<1>{0, num_gpus - 1});
  runtime->execute_index_space(ctx, Legion::IndexLauncher{
