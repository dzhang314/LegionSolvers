...
enum TaskIDs : Legion::TaskID { CHILD_TASK_ID, TOP_LEVEL_TASK_ID };
...
void top_level_task(
    ...
    std::cout << "Hello from the top-level task!" << std::endl;
    {
        Legion::TaskLauncher launcher{
            CHILD_TASK_ID, Legion::TaskArgument{nullptr, 0}};
...
int main(int argc, char **argv) {
    ...
    LegionSolvers::preregister_task<child_task>(
        CHILD_TASK_ID, "child", TaskFlags::REPLICABLE | TaskFlags::LEAF
    );
