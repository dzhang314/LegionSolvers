    ...
    class COOMatrix : public SparseMatrix<KERNEL_DIM, DOMAIN_DIM, RANGE_DIM, ENTRY_T> {
      ...
      public:
        ...
        kernel_partition_from_domain_partition(Legion::IndexPartitionT<DOMAIN_DIM> domain_partition,
            ...
            const Legion::IndexPartition result = rt->create_partition_by_preimage(ctx, domain_partition, matrix_region,
                                                                                   matrix_region, fid_j, color_space);
        ...
        kernel_partition_from_range_partition(Legion::IndexPartitionT<RANGE_DIM> range_partition,
            ...
            const Legion::IndexPartition result = rt->create_partition_by_preimage(ctx, range_partition, matrix_region,
                                                                                   matrix_region, fid_i, color_space);
            assert(result.get_dim() == KERNEL_DIM);
            return Legion::IndexPartitionT<KERNEL_DIM>{result};
        }
        ...
        explicit COOMatrix(Legion::LogicalRegionT<KERNEL_DIM> matrix_region,
            ...
            : matrix_region(matrix_region), fid_i(fid_i), fid_j(fid_j), fid_entry(fid_entry),
              input_partition(input_partition), output_partition(output_partition) {

            const auto kernel_domain_partition = kernel_partition_from_domain_partition(input_partition, ctx, rt);

            const auto kernel_range_partition = kernel_partition_from_range_partition(output_partition, ctx, rt);
        ...
        virtual void matvec(Legion::LogicalRegion output_vector,
            ...
            for (const auto [input_color, output_color] : nonempty_tiles) {
                ...
                const auto input_subregion = rt->get_logical_subregion_by_color(
                    rt->get_logical_partition(input_vector, input_partition), input_color);
