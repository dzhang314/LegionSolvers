...
void top_level_task(const Legion::Task *,
    ...
    const auto domain_space = rt->create_index_space(ctx,
        VectorRect{0, grid_size - 1});

    const auto range_space = rt->create_index_space(ctx,
        VectorRect{0, grid_size - 1});

    const auto range_color_space = rt->create_index_space(ctx,
        VectorColorRect{0, num_range_pieces - 1});
...

    {
        ...
        const auto rowptr_region = rt->create_logical_region(ctx,
            range_space, rowptr_field_space);

        {
            const typename
            LegionSolvers::FillCSRNegativeLaplacian1DTask<ENTRY_T>::Args args{
                FID_COL, FID_ENTRY, grid_size};
            Legion::TaskLauncher launcher{
                LegionSolvers::FillCSRNegativeLaplacian1DTask<ENTRY_T>::task_id,
                Legion::TaskArgument{&args, sizeof(args)}};
            launcher.map_id = LegionSolvers::LEGION_SOLVERS_MAPPER_ID;
            launcher.add_region_requirement(Legion::RegionRequirement{
                matrix_region, LEGION_WRITE_DISCARD,
                LEGION_EXCLUSIVE, matrix_region});
        ...
        {
            const typename
            LegionSolvers::FillCSRNegativeLaplacian1DRowptrTask<ENTRY_T>::Args args{
                FID_ROWPTR, grid_size};
            Legion::TaskLauncher launcher{
                LegionSolvers::FillCSRNegativeLaplacian1DRowptrTask<ENTRY_T>::task_id,
                Legion::TaskArgument{&args, sizeof(args)}};
            launcher.map_id = LegionSolvers::LEGION_SOLVERS_MAPPER_ID;
            launcher.add_region_requirement(Legion::RegionRequirement{
                rowptr_region, LEGION_WRITE_DISCARD,
                LEGION_EXCLUSIVE, rowptr_region});
            launcher.add_field(0, FID_ROWPTR);
            rt->execute_task(ctx, launcher);
        }

        LegionSolvers::CSRMatrix<ENTRY_T> csr_matrix{
            ctx, rt, matrix_region, FID_COL, FID_ENTRY, rowptr_region, FID_ROWPTR};
